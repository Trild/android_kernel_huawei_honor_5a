	...
	else if(state == CMDQ_ESD_CHECK_CMP)
	{
		...
		for(i = 0; i < 3; i++)
		{
				...
				&& (read_data1.byte2 == dsi_params->lcm_esd_check_table[i].para_list[2]))
			{
				...
				printk("esd_check: %#x, %#x, %#x, %#x, %#x, %#x, size = %d\n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]
					, read_data1.byte2,dsi_params->lcm_esd_check_table[i].para_list[2], sizeof(dsi_params->lcm_esd_check_table[i].para_list)/sizeof(unsigned char));
				ret = 0; // esd pass
			}
			else
			{
				printk("esd_check1: %#x, %#x, %#x, %#x, %#x, %#x, size = %d\n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]
					, read_data1.byte2,dsi_params->lcm_esd_check_table[i].para_list[2], sizeof(dsi_params->lcm_esd_check_table[i].para_list)/sizeof(unsigned char));
				ret = 1; //  esd fail
				break;
			}
