...
extern int read_otp_date_once_flag;
...
static int Get_Main_Camera_Id(void)
{
    ...
    LOG_INF("ov13850qt_Get_Main_Camera_Id cam_id = 0x%x",cam_id);
    return cam_id;
}
kal_uint8 readOtpSuccessFlag = 0;
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		read_otp_date_once_flag = 1;
		readOtpSuccessFlag = 1;
	 	 write_cmos_sensor(0x0100, 0x01);//stream on
		OV13850_read_otp(&OV13850_otp,imgsensor.i2c_write_id);
                return ERROR_NONE;
            }
...
static kal_uint32 open(void)
{
    ...
    sensor_init();
	  //for OTP
#ifdef OV13850_CHECK_OTP
	  printk("liyongjian >>> kal_uint32 open read_otp_date_once_flag = %d,readOtpSuccessFlag = %d\n",read_otp_date_once_flag,readOtpSuccessFlag);
	 OV13850_apply_otp(&OV13850_otp);
#endif
	  printk("lili>>>>this is oflim_OV13850\n");
    spin_lock(&imgsensor_drv_lock);
