...
#include "ov13850qt_otp.h"

#include "ov13850qtmipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "OV13850QT_camera_sensor"
#define LOG_1 LOG_INF("OV13850QT,MIPI 4LANE\n")
...
int ov13850qt_chip_ver = OV13850QT_R2A;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13850QT_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define OV13850_CHECK_OTP
#ifdef OV13850_CHECK_OTP
static struct 	otp_struct OV13850_otp;
#endif
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (ov13850qt_chip_ver == OV13850QT_R1A)
	{
		...
		//SENSORDB("[OV13850QTGain2Reg]: isp gain:%d,sensor gain:0x%x\n",iGain,iReg);
	}
	else if(ov13850qt_chip_ver == OV13850QT_R2A)
	{
	...
	return iReg;//ov13850qt. sensorGlobalGain
}
...
static void preview_setting(void)
{
    write_cmos_sensor(0x0100, 0x00);//
		...
		write_cmos_sensor(0x3807, 0x4b);
		write_cmos_sensor(0x3808, 0x08);
		write_cmos_sensor(0x3809, 0x30);//0x40
		write_cmos_sensor(0x380a, 0x06);
		write_cmos_sensor(0x380b, 0x10);//0x20
...
/*capture_first_flag & pre_currefps for resolve ov13850qtr2a 
...
if (capture_first_flag == 0)
{
    ...
    if (currefps == 150) { //15fps for PIP
        //@@full_132PCLK_24.75
			...
			write_cmos_sensor(0x3809, 0x60);// 0x80
			write_cmos_sensor(0x380a, 0x0c);//
			write_cmos_sensor(0x380b, 0x20);// 0x40
...
*
*************************************************************************/
#define  GPIO_MCIN_CMAERA_ID (GPIO7 | 0x80000000)

static int Get_Main_Camera_Id(void)
{
    int cam_id = 0;
	mt_set_gpio_mode(GPIO_MCIN_CMAERA_ID, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_MCIN_CMAERA_ID, GPIO_DIR_IN);
    cam_id = mt_get_gpio_in(GPIO_MCIN_CMAERA_ID);
    LOG_INF("liyongjian>>>> ov13850qt_Get_Main_Camera_Id cam_id = 0x%x",cam_id);
    return cam_id;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
	     ...
	     LOG_INF("liyongjian>>>> get_imgsensor_id retry = %d ,sensor_id = %d ,OV13850QT_SENSOR_ID = %d\n",retry,*sensor_id,OV13850QT_SENSOR_ID);
            main_camera_id = Get_Main_Camera_Id();
            if ((*sensor_id == imgsensor_info.sensor_id)&&(main_camera_id == 1)) {
                
	 	 write_cmos_sensor(0x0100, 0x01);//stream on
#ifdef OV13850_CHECK_OTP		
		read_otp(&OV13850_otp,imgsensor.i2c_write_id);
		LOG_INF("liyongjian>>>>get_imgsensor_id Read sensor id  success! i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
#endif
                return ERROR_NONE;
            }
            LOG_INF("liyongjian>>>>get_imgsensor_id Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    }
    if ((*sensor_id != imgsensor_info.sensor_id)||(main_camera_id == 0)) {
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id()+1;
		LOG_INF("liyongjian>>>> get_imgsensor_id retry = %d ,sensor_id = %d ,OV13850QT_SENSOR_ID = %d\n",retry,sensor_id,imgsensor_info.sensor_id);
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("liyongjian>>>open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("liyongjian>>>open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
    ...
    if (imgsensor_info.sensor_id != sensor_id)
		...
		{
				LOG_INF("----R1A---- \n");
				ov13850qt_chip_ver = OV13850QT_R1A;
		}else if((read_cmos_sensor(0x302a))==0xb2)
		{
				LOG_INF("----R2A---- \n");
				ov13850qt_chip_ver = OV13850QT_R2A;
		}
    /* initail sequence write in  */
    sensor_init();
	  //for OTP
#ifdef OV13850_CHECK_OTP
	 apply_otp(&OV13850_otp);
	 printk("liyongjian>>>>this is Qtech_OV13850\n");
#endif
...
UINT32 OV13850QT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    OV13850QT_MIPI_RAW_SensorInit    */
