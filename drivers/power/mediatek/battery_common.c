...
#define HQ_GET_BAT_TEMP _IOW('k',13,int)
#endif

#define FEATURE_HUAWEI_MAINTENANCE_TEST
#ifdef FEATURE_HUAWEI_MAINTENANCE_TEST
/* used for temperature close feature */
static int g_temp_detect_closed = 0;
#define NORMAL_TEMP					25
#define TEMP_DETECT_CLOSE			_IOW('k', 14, int)
/* 
 * used for manual set charging current feature
 * used [Set_Charger_Current] API
 */
#define MANUAL_SET_CURRENT_ENABLE	_IOW('k', 15, int)
#define MANUAL_SET_CURRENT			_IOW('k', 16, int)
#endif
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY_FCC:
		val->intval = gFG_BATT_CAPACITY;
		break;
	case POWER_SUPPLY_PROP_CAPACITY_RM:
		val->intval = (gFG_BATT_CAPACITY*BMT_status.SOC)/100;
		break;
...
#endif
#ifdef POST_99TO100_CHARGING_TIME
/*zcy add in 20151102, for 99% , charging time counter*/
...
static kal_bool mt_battery_nPercent_tracking_check(void)
{
	...
	} else if ((BMT_status.UI_SOC > three_percent_check_point) && (BMT_status.ZCV <= three_percent_zcv)) 
	{
		resetBatteryMeter = KAL_TRUE;
		battery_log(BAT_LOG_CRTI,
				    "[3Percent] sync 3percent ZCV %d <= BMT_status.nPercent_ZCV %d and UI SOC=%d, then keep %d.\n",
     ...
     )
 {
	...
	if (resetBatteryMeter == KAL_TRUE) {
		...
		if (bat_is_recharging_phase() == KAL_TRUE) {
			// DTS:CPM2016050900085 zhaojunyao add begin 20160525 
			//BMT_status.UI_SOC = 100;
			// close set recharge_soc 100, control soc in 100percent
			// DTS:CPM2016050900085 zhaojunyao add end
			battery_log(BAT_LOG_CRTI, "[recharging] UI_SOC=%d, SOC=%d, state:%d, %d\n",
					    BMT_status.UI_SOC, BMT_status.SOC, BMT_status.bat_in_recharging_state ,BMT_status.bat_full);
                }
...
PMU_STATUS do_batt_temp_state_machine(void)
{
	...
	if (BMT_status.temperature < MIN_CHARGE_TEMPERATURE) {
		...
		battery_log(BAT_LOG_CRTI,"[do_batt_temp_state_machine]zcy-temperature<%d\n",MIN_CHARGE_TEMPERATURE);
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI,"zcy bat_vol=%d \n",bat_vol);
...
static PMU_STATUS mt_battery_CheckBatteryTemp(void)
{
	...
	if (do_batt_temp_state_machine() == PMU_STATUS_FAIL) {
		...
		battery_log(BAT_LOG_CRTI,"[mt_battery_CheckBatteryTemp]zcy-do_batt_temp_state_machine() == PMU_STATUS_FAIL\n");
		status = PMU_STATUS_FAIL;
	}
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	#endif
	//battery_log(BAT_LOG_CRTI, "zjy1 [BATTERY] Charger voltage %d \r\n", BMT_status.charger_vol);
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	if (mt_battery_CheckBatteryTemp() != PMU_STATUS_OK) {
		BMT_status.bat_charging_state = CHR_ERROR;
		battery_log(BAT_LOG_CRTI,"[mt_battery_CheckBatteryStatus]zcy-BMT_status.bat_charging_state = CHR_ERROR\n");
		return;
	}
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if ((BMT_status.temperature>=TEMP_POS_45_THRESHOLD) && (BMT_status.temperature<=TEMP_POS_60_THRESHOLD) && (BMT_status.bat_vol > 4100)) {
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) at [45, 60)\n",
				    BMT_status.temperature);
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range (,-20] (too low)\n",
				    BMT_status.temperature);
	}
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
...
static void mt_battery_notify_current_check(void)
{
	...
	{
		if(bat_vol<BMT_status.bat_vol)
		{
			bat_vol=BMT_status.bat_vol;
		}
		else if(bat_vol-BMT_status.bat_vol > 50)
		{
			bat_vol=BMT_status.bat_vol+25;
			g_BatteryNotifyCode |= 0x0040;
			battery_log(BAT_LOG_FULL, "[BATTERY] BATTERY_NOTIFY_CASE_0006_CURRENT (%x)\n",
			    g_BatteryNotifyCode);
		}
	}
	else
...
extern kal_int32 gFG_capacity_by_c;
...
#if 0  //#ifdef BATTERY_CDP_WORKAROUND
...
#ifdef FEATURE_HUAWEI_MAINTENANCE_TEST
	case TEMP_DETECT_CLOSE:
		/* 1: close temperature detect feature, 0: open this feature */
		user_data_addr = (int *)arg;
		g_temp_detect_closed = *user_data_addr;

		battery_log(BAT_LOG_CRTI, "Temperature detect %s\n", 
						g_temp_detect_closed?"closed":"opened");
		break;

	case MANUAL_SET_CURRENT_ENABLE:
		/* 1: enable manual set current feature, 0: disable this feature */
		charging_level_data[0] = 0;

		user_data_addr = (int *)arg;
		g_ftm_battery_flag = *user_data_addr;

		battery_log(BAT_LOG_CRTI, "The feature of manual set current %s\n", 
						g_ftm_battery_flag?"enable":"disable");
		break;
	case MANUAL_SET_CURRENT:
		/* set current*/
		user_data_addr = (int *)arg;
		ret = copy_from_user(charging_level_data, user_data_addr, 4);
		if (charging_level_data[0] > AC_CHARGER_CURRENT)
			charging_level_data[0] = AC_CHARGER_CURRENT;

		battery_log(BAT_LOG_CRTI, "Manual set current: %d ma\n", charging_level_data[0]);
		break;
#endif
			...
			g_runin_test_flag = 1;			 
                       ...
                       wake_up_bat();
                       break;                                                   
                       case HQ_GET_BAT_TEMP://zcy add 20151102 end
                       user_data_addr = (int *)arg;
                       ret = copy_from_user(adc_in_data, user_data_addr, 8);
